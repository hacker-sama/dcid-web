...
class _AdminScreenState extends ConsumerState<AdminScreen> {
  ...
  AdminUserFilter _selectedRoleFilter = AdminUserFilter.all;
  ...
  Future<void> _fetchUsers() async {
    ...
    try {
      ...
      final list = await repo.listUsers(size: 100);
      setState(() {
        _allUsers = list;
        _users = _filteredUsers(list);
        _isLoading = false;
      });
    } catch (e) {
      setState(() {
  ...
  List<AppUser> _filteredUsers(List<AppUser> users) {
    return filterAdminUsers(
      users,
      filter: _selectedRoleFilter,
      search: _searchController.text,
    );
  }

  void _applyFilters() {
    setState(() => _users = _filteredUsers(_allUsers));
  }

  void _selectRoleFilter(AdminUserFilter filter) {
    if (_selectedRoleFilter == filter) return;
    setState(() {
      _selectedRoleFilter = filter;
      _users = _filteredUsers(_allUsers);
    });
  }
    ...
    showDialog(
      ...
      builder: (ctx) {
        ...
        return AlertDialog(
          ...
          actions: [
            TextButton(onPressed: () => nav.pop(), child: Text(strings.cancel)),
            FilledButton(
              onPressed: () async {
            ...
            FilledButton(
              onPressed: () async {
                ...
                try {
                  ...
                  nav.pop();
                  messenger.showSnackBar(
                    SnackBar(content: Text(strings.changePasswordSuccess)),
                  );
                } catch (e) {
                  messenger.showSnackBar(SnackBar(content: Text('Error: $e')));
                }
  ...
  Widget _buildStatCard({
    ...
    return Semantics(
      button: true,
      selected: selected,
      label: 'Filter user list by $title',
      child: Card(
  ...
  Widget build(BuildContext context) {
    ...
    final operators = _allUsers
        .where((u) => u.role == UserRole.operatorRole)
        .length;
    final engineers = _allUsers
                      ...
                      builder: (context, constraints) {
                        ...
                        final cards = [
                          _buildStatCard(
                            ...
                            scheme: scheme,
                            selected:
                                _selectedRoleFilter == AdminUserFilter.all,
                            onTap: () => _selectRoleFilter(AdminUserFilter.all),
                          ),
                          _buildStatCard(
                            title: 'Operators',
                            value: '$operators',
                            ...
                            selected:
                                _selectedRoleFilter ==
                                AdminUserFilter.operatorRole,
                            onTap: () =>
                                _selectRoleFilter(AdminUserFilter.operatorRole),
                          ),
                          _buildStatCard(
                            title: 'Engineers',
                            value: '$engineers',
                            ...
                            scheme: scheme,
                            selected:
                                _selectedRoleFilter == AdminUserFilter.engineer,
                            onTap: () =>
                                _selectRoleFilter(AdminUserFilter.engineer),
                          ),
                          _buildStatCard(
                            title: 'Admins & QA',
                            value: '$admins',
                            ...
                            selected:
                                _selectedRoleFilter ==
                                AdminUserFilter.adminsAndQa,
                            onTap: () =>
                                _selectRoleFilter(AdminUserFilter.adminsAndQa),
                          ),
                        ];
                        ...
                        child: LayoutBuilder(
                          builder: (context, constraints) {
                            ...
                            final roleDropdown = SizedBox(
                              ...
                              height: 44,
                              child: DropdownButtonFormField<AdminUserFilter>(
                                key: ValueKey(_selectedRoleFilter),
                                ...
                                items: [
                                  ...
                                  DropdownMenuItem(
                                    value: AdminUserFilter.operatorRole,
                                    child: Text(
                                      UserRole.operatorRole.localized(strings),
                                      style: const TextStyle(fontSize: 13),
                                    ),
                                  ),
                                  DropdownMenuItem(
                                    value: AdminUserFilter.engineer,
                                    child: Text(
                                      UserRole.engineer.localized(strings),
                                      style: const TextStyle(fontSize: 13),
                                    ),
                                  ),
                                  const DropdownMenuItem(
                                    value: AdminUserFilter.adminsAndQa,
                                    child: Text(
                                      'Admins & QA',
                                      style: TextStyle(fontSize: 13),
                                    ),
                                  ...
                                  DropdownMenuItem(
                                    value: AdminUserFilter.qaAdmin,
                                    child: Text(
                                      UserRole.qaAdmin.localized(strings),
                                      style: const TextStyle(fontSize: 13),
                                    ),
                                  ),
                                  DropdownMenuItem(
                                    ...
                                    child: Text(
                                      UserRole.admin.localized(strings),
                                      style: const TextStyle(fontSize: 13),
                                    ),
                                  ),
                                ],
                                onChanged: (v) {
                                  if (v != null) _selectRoleFilter(v);
                                },
                              ),
                            );
