...
def chunk_pages(
    ...
    Đặc biệt:
    - Nếu trang có chứa Visual Crops từ Qwen2-VL 2B, biến mỗi Visual Crop thành 1 Chunk riêng
      có gắn `image_path` để Frontend có thể lấy trực tiếp render lên UI!
    """
    chunks: List[Chunk] = []
    global_idx = 0

    for page in pages:
        # 1. Tạo các Chunk cho Visual Crops (nếu có)
        if page.image_crops:
                ...
                if crop.visual_caption:
                    ...
                    chunks.append(
                        Chunk(
                            ...
                            text=crop_text,
                            bbox=crop.bbox,
                            image_path=crop.image_path,  # Gắn image_path vào metadata!
                            snippet=crop.visual_caption[:150],
                        )
                    )
                    global_idx += 1

        # 2. Tạo các Chunk cho Text thường (Sliding Window Layout-Aware)
