...
def get_openai_client():
    ...
    return OpenAI(
        base_url=os.getenv("LM_STUDIO_BASE_URL", lm_cfg.get("base_url", "http://localhost:11434/v1")),
...
def generate_vision_caption(
    ...
    Args:
        image_base64: Chuỗi Data URI Base64 dạng 'data:image/png;base64,...'
        prompt: Task-tuned prompt ngắn gọn (từ prompt_templates.py)
        model_name: Tên model vision (mặc định lấy từ config.yaml)

    Returns:
        Mô tả nội dung ảnh / Văn bản trích xuất từ Qwen2-VL 2B.
    """
    cfg = _load_config()
...
def generate_rag_answer(
    ...
    try:
        ...
        # Clean think tags nếu model dạng reasoning
