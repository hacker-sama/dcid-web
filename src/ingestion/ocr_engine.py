...
def _detect_page_drawings_and_images(
    ...
    for img_info in image_list:
        ...
        for img_rect in page.get_image_rects(xref):
            # Scale rect sang pixel space của pixmap
...
def process_pdf_pages(
    ...
    Cơ chế Pure-Text Vision Skip:
    - Nếu trang PDF không chứa sơ đồ/hình ảnh (tài liệu chữ thuần), tự động SKIP gọi Qwen2-VL 2B.
    - Giúp tiết kiệm thời gian xử lý và giảm latency tối đa.

    Visual Bounding Box Chunking:
    - Cắt độc lập các vùng sơ đồ/bản vẽ/bảng thành file PNG ở uploads/crops/.
    - Truyền ảnh crop vào Qwen2-VL 2B với prompt ngắn gọn để lấy caption.
    - Lưu `image_path` để lưu vào Vector Store cho Frontend UI.
    """
    ...
    try:
        for i, page in enumerate(doc, start=1):
            ...
            # Trích xuất native text trước (PDF Word/CAD chuẩn)
            ...
            # ── PHÁT HIỆN HÌNH ẢNH & KIỂM TRA PURE-TEXT VISION SKIP ──
            ...
            if enable_vision and has_visual and visual_bboxes:
                ...
                for crop_idx, bx in enumerate(visual_bboxes[:3]):  # Tối đa 3 crop lớn nhất mỗi trang
                    crop_img = helpers.crop_image_region(pil_page_img, bx, padding=15)
                    # Lưu file ảnh crop vào uploads/crops/
                    ...
                    # Chuyển ảnh crop sang base64
                    base64_str = helpers.convert_image_to_base64(crop_img)

                    # Gọi Qwen2-VL-2B với prompt ngắn gọn (Off-line visual captioner)
